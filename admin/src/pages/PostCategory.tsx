...
import { usePostCategories } from "../api/querys";
import CreatPostCategory from "../forms/CreatPostCategory";

const PostCategory: React.FC = () => {
  const [isPostCategoryForm, setPostCategoryForm] =
    useState<PostCategoryStateType>({
      creat: false,
      updateId: "",
      updateData: "",
    });
  ...
  const { isPending, isError, data, error, refetch } = usePostCategories();
  ...
  const updateCategory = (category: GetPostCatgeoryData) => {
    setPostCategoryForm((prev) => ({
      ...prev,
  ...
  return (
    <>
      {(isPostCategoryForm.creat || isPostCategoryForm.updateId) && (
        <CreatPostCategory
          isPostCategoryForm={isPostCategoryForm}
          setPostCategoryForm={setPostCategoryForm}
          refetch={refetch}
          clear={clearFileds}
        />
      )}
  ...
  h-full
      ...
      rounded-md  font-philosopher max-w-full w-full shadow-md`}
        >
          ...
          >
            <section className="grid gap-4 p-2 pb-2 min-w-[600px] font-medium  grid-cols-customePostCategory text-[#DEE1E2] md:font-semibold bg-[#1A1A1A]">
            ...
            <div className=" h-[380px] overflow-y-auto [&::-webkit-scrollbar]:hidden min-w-[600px] bg-[#252525]">
              {
                isPending ? (
                  ...
                  currentCategory?.map((category, i) => (
                    <section
                      key={i}
                      className="grid items-center gap-6 py-2 pl-6 pr-4 border-t-2 text-[#DEE1E2] border-[#1A1A1A] grid-cols-customePostCategory group hover:bg-[#2c2c2c]"
                    >
                      <span>{i + 1}</span>
