...
const update = async (req, res) => {
  try {
    const prosCons = await ProsCons.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
    if (!prosCons) {
      return res
        .status(404)
        .json({ success: false, message: "ProsCons not found" });
    }
...
const Delete = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "ProsCons deleted successfully" });
  } catch (error) {
    res.status(400).json({ success: false, error: error.message });
  }
};
...
const getAllProsconController = async (req, res) => {
  try {
    const prosCons = await ProsCons.find();
    res.status(200).json({ success: true, data: prosCons });
  } catch (error) {
    res.status(400).json({ success: false, error: error.message });
  }
